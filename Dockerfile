...
ENV RAILS_ENV=production \
  ...
  POSTGRES_SHARED_BUFFERS=8MB \
  POSTGRES_EFFECTIVE_CACHE_SIZE=32MB \
  POSTGRES_MAINTENANCE_WORK_MEM=4MB \
  POSTGRES_WORK_MEM=512kB \
  POSTGRES_TEMP_BUFFERS=512kB \
  POSTGRES_WAL_BUFFERS=128kB \
  POSTGRES_MAX_CONNECTIONS=8 \
  POSTGRES_AUTOVACUUM_MAX_WORKERS=1 \
  POSTGRES_AUTOVACUUM_WORKER_SLOTS=1 \
  POSTGRES_MAX_WORKER_PROCESSES=1 \
  POSTGRES_MAX_PARALLEL_WORKERS=0 \
  POSTGRES_MAX_PARALLEL_WORKERS_PER_GATHER=0 \
  POSTGRES_MAX_PARALLEL_MAINTENANCE_WORKERS=0 \
  POSTGRES_MAX_WAL_SENDERS=0 \
  POSTGRES_MAX_REPLICATION_SLOTS=0 \
  POSTGRES_IO_METHOD=sync \
  POSTGRES_EFFECTIVE_IO_CONCURRENCY=0 \
  POSTGRES_MAINTENANCE_IO_CONCURRENCY=0 \
  POSTGRES_JIT=off \
  POSTGRES_TRACK_ACTIVITY_QUERY_SIZE=128 \
  BUNDLE_DEPLOYMENT=true \
  BUNDLE_WITHOUT=development:test \
  BUNDLE_PATH=/usr/local/bundle \
  GEM_HOME=/usr/local/bundle \
  ...
  MALLOC_CONF=narenas:1,background_thread:false,thp:never,dirty_decay_ms:0,muzzy_decay_ms:0
